...
const tmdbPageHandler = async (
  ...
  const [_componentId, invoker, action, messageId] = customId!.split("_");
...
const generateMovieEmbed = async (result: TMDBResult, type: string) => {
  ...
  if (fullDetails.production_companies.length > 0) {
    possibleOptionalFields.push({
      ...
      value: fullDetails.production_companies
        .map((x: any) => x.name)
        .join("\n"),
      inline: true,
    });
...
const generateWatchEmbed = async (result: TMDBResult, type: string) => {
  ...
  if (providerRegion?.flatrate?.length > 0) {
    possibleOptionalFields.push({
      name: "Streaming",
      value: providerRegion.flatrate
        .map((x: any) => `- ${x.provider_name}`)
        .join("\n"),
      inline: true,
    });
  }
...
const generateTMDBPages = (pages: TMDBResult[]): SelectOption[] => {
  return pages.map((page, index) => {
    const releaseYear = new Date(
      page.release_date || page.first_air_date,
    ).getFullYear();
...
const parseOptions = (interactionOptions: any) => {
  const titleOption = interactionOptions.find(
    (option: { name: string }) => option.name === "title",
  );
  const title: string = titleOption ? (titleOption.value as string) : "";

  const yearOption = interactionOptions.find(
    (option: { name: string }) => option.name === "year",
  );
