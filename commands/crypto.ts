...
export const sendCryptoEmbed = async (bot: Bot, interaction: Interaction) => {
  ...
  const symbol: string = symbolOption
    ? String(symbolOption.value).toLowerCase()
    : "";
  ...
  // console.log(
  //   generatePageButtons("crypto", pageData, internalMessageId)[0].components[1],
  // );

  const components = [
    ...(coinsMatchingSymbol.length > 1
      ? generatePageButtons("crypto", pageData, internalMessageId)
      : []),
...
const fetchCryptoMap = async () => {
  ...
  const coinMap: cgCoin[] = (await request.json()) as any;
